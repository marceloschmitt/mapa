...
| 3 | `analisar_frequencia.py` | Normaliza frequência por aluno/disciplina | `resposta_alunos.json` | `data/json/tabela_frequencia.json` |
...
| 7 | `importar_chamadas.py` | Última aula / histórico de chamadas | `resposta_alunos.json`, BD (coleta) | BD (`disciplina_ultima_aula`, `disciplina_chamadas`) |
| 8 | `gerar_alarmes.py` | Regras de risco de evasão | BD (coleta + faltas), `config/consultas.json` | BD (`alarmes`) |
